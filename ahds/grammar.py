...
_hyper_surface_entities = b'|'.join(_dict_iter_keys(_hyper_surface_file))
...
_file_format_match = re.compile(b'^\s*#\s*(?P<format>AmiraMesh|HyperSurface)(?:\s+|$)')
...
_stream_delimiters = [
    ...
    re.compile(
        b"(?:^|\n)\s*(?P<stream>(?:"+_hyper_surface_entities+ b"))(?:\s+(?:(?P<count>\d+)|(?P<string>(?:\w|[^\n{])+)))?(?P<group>\s*(?:\n\s*)?\{\s*\n)?\s*\n"
    ),
    ...
    re.compile(b"(?P<stop>\s*\}(?:(?:\s|\n)*\{)?\n|(?:" + _hyper_surface_entities + b"))", re.I),
    re.compile(b'}(?:\s*{)?')
]
# pattern for locating stream_header and comments in ascii HyperSurface type file
_stream_delimiters[3] = re.compile(_stream_delimiters[3].pattern + b'|' + _stream_delimiters[2].pattern,re.S)
# pattern for locating stream_header and comments in ascii AmiraMesh type file
_stream_delimiters[2] = re.compile(_stream_delimiters[0].pattern + b'|' + _stream_delimiters[2].pattern)
_empty_stream_data = b''
_split_item_counter = re.compile('^(?P<basename>(?:\D|\d+(?=\D))+)(?P<itemcount>\d+)$')
...
def next_amiramesh_binary_stream(fhnd,stream_bytes=32768,stream_data = _empty_stream_data,**kwargs):
    ...
    returns the encoded data for the current stream, the bytes remaining from current read
        the index of the next stream if any and the offste of the next stream
    """
...
def collect_hypersurface_ascii_stream(fhnd,count,stream_data,stream_bytes = 32768,drop_data = False,**kwargs):
    if not stream_data:
        stream_data = fhnd.read(stream_bytes - len(stream_data) if stream_bytes > _rescan_overlap else _rescan_overlap)
    at_end_of_file = stream_data
    stream_data = b''
    continue_scanning_at = 0
    next_stream = None
    ...
    return (None if drop_data else (stream_data[:next_stream.start()] if next_stream is not None else stream_data)),_empty_stream_data
...
def parse_hypersurface_data(fhnd,parsed_data = dict(),verbose = False,stream_bytes=32768,stream_data = _empty_stream_data,**kwargs):
    ...
    parsed_item_id = 0
    for parsed_item_id,parsed_item in enumerate(parsed_data):
    ...
    if not isinstance(data_format,str) or not data_format:
        raise ValueError("'parsed_data' does not represent valid HyperSurface data_definitions")
    ...
    parse_hypersurface_data.remaining_bytes = stream_data
    stream_data = b''
    def iter_hypersurface_stream():
        ...
        while True:
            if len(parse_hypersurface_data.remaining_bytes) < _rescan_overlap:
                at_end_of_file = fhnd.read(stream_bytes)
                if not at_end_of_file:
                    return
                parse_hypersurface_data.remaining_bytes += at_end_of_file
            yield parse_hypersurface_data.remaining_bytes
    ...
    for stream_data in iter_hypersurface_stream():
        ...
        if stream_name is None:
            raise Exception("'{}' unknown HyperSurface file stream".format(_decode_string(stream_name)))
        ...
        if stream_info[3] == 'group':
            if max_items - item_count > 0:
                raise Exception("{} missing items of '{}' group read".format(item_count,group_name))
            if group_level > 0 or stream_info[0] is not _group_array_declarations:
                raise Exception("HyperSurface sub groups not suported")
            if num_items is None:
                raise Exception("Itemcount not readable on stream '{}'".format(_decode_string(stream_name)))
            ...
            if max_items > 0:
                ...
                raise Exception("{} group is mandatory".format(stream_info[1]))
            parse_hypersurface_data.remaining_bytes = stream_data[next_stream.end():]
            continue_scanning_at = 0
            continue
        if stream_info[2] is not None:
            if num_items is not None:
                ...
                encoded_data,parse_hypersurface_data.remaining_bytes = extract_stream_data(fhnd,num_bytes,stream_data[next_stream.end():],stream_bytes = stream_bytes,**kwargs)
            else:
                encoded_data,parse_hypersurface_data.remaining_bytes = _decode_string(next_stream.group("string")),stream_data[next_stream.end():]
        elif num_items is not None:
            encoded_data,parse_hypersurface_data.remaining_bytes = int(num_items),stream_data[next_stream.end():]
            num_items = None
        else:
            encoded_data,parse_hypersurface_data.remaining_bytes = _decode_string(next_stream.group("string")),stream_data[next_stream.end():]
        continue_scanning_at = 0
        ...
        if array_id is not stream_info[0]:
            raise Exception("'{}' stream not expected on '{}' group".format(stream_name,array_name))
        data_definitions.append(
            dict(
...
def parse_header(data, verbose=False,  **kwargs):
    ...
    meta_declarations = collections.OrderedDict()
    array_declarations = None
    for array_declarations in (
        declaration_item
        for declaration_item in (
            parsed_item.get('array_declarations',None)
            for parsed_item in parsed_data
        )
        if declaration_item is not None
    ):
        break
    if verbose:
...
def get_parsed_data(fn, **kwargs):
    ...
    with open(fn,'rb') as fhnd:
        ...
        parsed_data = parse_header(data, **kwargs)
