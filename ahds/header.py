...
try:
    from .core import _dict_iter_keys, _dict_iter_items, _dict_iter_values, Block, _AnyBlockProxy, deprecated, \
        ListBlock, REFACTOR
except:
    from core import _dict_iter_keys, _dict_iter_items, _dict_iter_values, Block, _AnyBlockProxy, deprecated, ListBlock, \
        REFACTOR
import numpy as np
import re
    ...
    if REFACTOR:
        ...
        def _load_declarations(self, block_data):
            ...
            for decl in block_data:
                ...
                block.add_attr('dimension', decl['array_dimension'])
                self.add_attr(block)
            return self
        ...
        def _load_definitions(self, block_data):
            ...
            for defn in block_data:
                block = Block(defn['data_name'])
                block.add_attr('data_index', defn['data_index'])
                if 'data_dimension' in defn:
                    block.add_attr('data_dimension', defn['data_dimension'])
                block.add_attr('data_type', defn['data_type'])
                # block.add_attr('array', defn['array_reference'])
        ...
        def _load_declarations(self, block_data):
            if len(block_data) < 1:
                ...
                raise ("array declarations found on file '{}' designated having '{}' file type".format(self.path,
                                                                                                       self.filetype))
            ...
            if _special_content in ['HxSpreadSheet']:
                ...
                if _numrows is not None:
                    ...
                    _check_dim = (
                        ...
                        _flatten,
                        lambda name: self._stream_loader.create_list_array(name, kind='spreadsheet',
                ...
                # _check_dim is a very bad idea; meaning of variables is lost!!!
                _check_dim = (
                    lambda dim: True,
                    AmiraHeaderError("strange"),
                    lambda name: name,
                    ...
                    try:
                        ...
                        if _rename is None:
                            ...
                            self._array_data_rename[_array_name] = {
                                0: ft.partial(_check_dim[4], formatter=(
